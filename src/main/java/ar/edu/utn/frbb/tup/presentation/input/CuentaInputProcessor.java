...
public class CuentaInputProcessor extends BaseInputProcessor{
    ...
    public void altaCuenta() {
        ...
        while (!tipoCuentaStr.equals("C") && !tipoCuentaStr.equals("A")) {
            System.out.println("Tipo de cuenta invalido. Ingrese C o A: ");
        ...
        if(tipoCuentaStr.equals("C")){
            ...
            cuenta.setMoneda(TipoMoneda.PESOS);  // Si es una cuenta corriente la moneda es PESOS
            cuenta.setBalance(0);
            cuenta.setFechaCreacion(LocalDate.now().atStartOfDay());
        } else {
            System.out.println("Elija el tipo de moneda de la cuenta - (P) Pesos (D) Dolares: ");
            String tipoMoneda = scanner.nextLine().toUpperCase();
            while (!tipoMoneda.equals("P") && !tipoMoneda.equals("D")) {
                System.out.println("Tipo de moneda invalido. Ingrese (P) Pesos o (D) Dolares: ");
